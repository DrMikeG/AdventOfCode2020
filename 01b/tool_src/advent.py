...
def mainTask():
    #argon_path = "C:\\workdir\\int\\argon"
    #newestBundle = findNewestBundle()
    #replaceFilesForTestPackageCreator(newestBundle,argon_path)
    #replaceFilesForArgonItself(newestBundle,argon_path)

    options = []

    input_path = "C:\\Users\\gibbens\\Documents\\Arduino\\AdventOfCode2020\\01a\\tool_src\\input.txt"
    ...
    valid = []
    # If two entrys sum to 2020, the 2020 - x should also be present in options.
    for x in options:
        for y in options:
            for z in options:
                if (z != y ) and (y != x):
